...
def do_chunk(dataset, state, task_specs, i1, i2):
    ...
    encoding = Encoding()
    return serialize(encoding.encode_list('task-part-cpu', task_parts), encoding)
...
class Executor(vaex.execution.Executor):
    def __init__(self, chunk_size=1_000_000):
        ...
        self.server = None
        self.do_chunk = self.remote(do_chunk)
        self.reduce = self.remote(reduce)
    ...
    async def execute_async(self, delay=False):
        ...
        while not cancelled:
            ...
            df = tasks_df[0].df
            state = df.state_get()
            if df.dataset not in self.remote_objects:
                self.remote_objects[df.dataset] = self.put(df.dataset)
            ...
            for chunk_index in range(chunk_count):
                ...
                task_parts = self.do_chunk(self.remote_objects[df.dataset], state, task_specs, i1, i2)
                if task_parts_previous is not None:
                    task_parts = self.reduce(task_parts_previous, task_parts)
                task_parts_previous = task_parts
            if delay:
                return task_parts
            task_parts = deserialize(self.get(task_parts), encoding)
            task_parts = encoding.decode_list('task-part-cpu', task_parts, df=df)
            for task, task_part in zip(tasks_df, task_parts):
                task.result = task_part.get_result()
